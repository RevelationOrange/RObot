...
wsFolder = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'wikiSearchData'
searchDataSections = {}
for fn in os.listdir(wsFolder):
    with open(sep.join([wsFolder, fn])) as jsf:
        sectionList = []
        for line in jsf:
            sectionList.append(json.loads(line))
        searchDataSections[fn] = copy.copy(sectionList)
...
def getItemKeys():
    ...
    argAssocs = {'madeOn': ['made on', 'station', 'workstation'], 'sellXP': ['sell xp', 'sale xp'],
                 ...
                 'craftTime': ['speed', 'craft speed', 'craft time', 'time'], 'ingredients': ['uses', 'needs', 'using'],
                 ...
                 'nfBuilds': ['used in building', 'building', 'buildings'], 'type': ['category'], 'rare': ['israre', 'is rare'],
...
def getCustomerKeys():
    argAssocs = {'color': ['hex code'], 'lvlReq': ['level required', 'shop level'], 'equips': ['can use', 'equip', 'klash'],
                 ...
                 'unlockedBy': ['prereq', 'building', 'unlocked by'], 'maxLvl': ['max level'], 'chClass': ['class']}
    baseKeys = ['color', 'id', 'lvlReq', 'name', 'startLvl', 'iTypes', 'maxLvl', 'chClass', 'unlockedBy', 'appealReq',
                'equips']
...
def getModuleKeys():
    ...
    numTypes = ['tier', 'levelReq', 'hammerCost', 'id', 'maxBuyable']
...
def getHuntKeys():
    ...
    baseKeys = ['comValue', 'minLvl', 'description', 'maxLvl', 'time', 'name', 'loots', 'id', 'picLink', 'xpPerLoot',
                'shopLvlReq']
    numTypes = ['comValue', 'minLvl', 'maxLvl', 'time', 'id', 'xpPerLoot', 'shopLvlReq']
    listTypes = []
    list2Types = ['loots']
    strTypes = list(set(numTypes + listTypes + list2Types) ^ set(baseKeys))
    argKeys = {}
...
def getAchievementKeys():
    ...
    baseKeys = ['name', 'requirement', 'reward']
    numTypes = []
    listTypes = ['reward']
    list2Types = []
    strTypes = list(set(numTypes + listTypes + list2Types) ^ set(baseKeys))
...
def wikiSearchV2(qs, show=True, outFile=dummyFile(), useThisData=0):
    ...
    baseSections = ['customers', 'hunts', 'improvements', 'items', 'modules',
                    'quests', 'workers', 'achievements', 'character_classes']
    ...
    if useThisData:
        searchData = useThisData
    else:
        searchData = searchDataSections[actualSection]
...
def multiSearch():
    ws = ['worker', 'tinkerer', 'armorer', 'sorc', 'enchanter']
    for w in ws:
        # break
        q = 'items, uses: not mithril eq gems crystal, worker: {} not leather, rrare: no'.format(w)
        ...
        for x in a:
            print(x['name'], x['ingredients'], x['madeBy'])
        print(len(a))

def quickSearch(q, names=True):
    # q = 'items, uses: gems not crystal mithril, rrare: no'
    # nonRares = wikiSearchV2(q)
    # q = 'items, uses: eq mithril or gems crystal'
    # tmpDum = dummyFile()
    recs = wikiSearchV2(q)
    # print(q)
    # print(len(recs))
    if names:
        anames = (([x['name'] for x in recs]))
        return ', '.join(anames)
    else:
        return recs
    # print(recs)
...
if __name__ == '__main__':
    # quests (event quests)
    # hunts = "Handouts, Street Peddler, Farming Fields, City Patrol, Castle Catacombs, Overgrown Ruins, Unexplored Mine, Hidden Passage, Volcano Mountain, Fairy's Grove, Troglodyte Cave, Tower Dungeon, Underground Hovel, Dangerous Fissure, Abandoned Sanctuary, Lighthouse Cellar, Desert Keep, Demon Cove, Stalagmite Den, Magical Tower, Fiery Volcano, Ancient Temple, Distant Lands, Mystical Pillars, Floating Castle, Haunted Forest, Dimensional Rift, Dark Abyss"
    # quests = "Gerbip Problem, Butterly Pests, Mushrhum Rampage, Mushrhum Rampage 2, Furry Spiders, Gerbip Problem 2, Losing Drums, Fairy Dusty, Losing Drums 2, Thieving Fox, Thieving Fox 2, Frock's Rocks, Captain Cacti's Crew, Frock's Rocks 2, Annoying Belicans, Charming Snakes, Scared-Kurtey, Scared-Kurtey 2, Definitely Not-a-Rat, Lost Explorers, Lost Explorers 2, Dangerous Slopes, Dangerous Slopes 2, Tricloptic Mayhem, Tricloptic Mayhem 2, Bugging Miners, The Abobinable, Bugging Miners 2, Summit Witch, Demon in the Mist, Phantom Shield, Bloglodyte Invasion?, Phantom Shield 2, Ninjusurtle, Snail With Mail, Ninjusurtle 2, Zombullying, Snail With Mail 2, Dizzy Gloats, Wolf Riders Riding, Leezard Leeder, Wolf Riders Riding 2, Old Bones, Leezard Leeder 2, Not-so Admirable Eagle"
    # hs = [x.replace(',','') for x in hunts.split(' ')]
    # qs = [x.replace(',','') for x in quests.split(' ')]
    # print(qs)
    baseSections = ['customers', 'hunts', 'improvements', 'items', 'modules',
                    'quests', 'workers', 'achievements', 'character_classes']
        # print()
    q = "items"
    a = quickSearch(q, False)
    vs = 'aeiou'
    v1s = []
    vowelsDict = {}
    for x in a:
        ...
        for c in vs:
            v += x['name'].count(c)
        if v in vowelsDict:
            vowelsDict[v].append(x)
        else:
            vowelsDict[v] = [x]
    print(vowelsDict.keys())
    vlvl = {}
    for n in vowelsDict:
        for x in (vowelsDict[n]):
            # vlist = [y for y in x['name'] if y in vs]
            # z = (len(set(vlist)))
            # if z == len(vs):
            #     print(x['name'])
            l = x['value']
            if n in vlvl:
                ...
                vlvl[n] = [l]
    for n in vlvl:
        print(n, "{:,}".format(sum(vlvl[n])/(1.*len(vlvl[n]))))
    # print(m)
    # a = quickSearch('quests')
    # for x in (a):
    #     print(x)
    # for x in qs:
    #     qu = q.format(x)
    #     a = quickSearch(qu, False)
    #     print(qu)
    #     print(a)
    #     print()
    ...
    # magic drink, health drink, soft potion, super potion, fortune scroll, shielding scroll, fireshield scroll, blaze scroll
    # q = 'items, uses: eq mithril, worker: sorc, rrare: no'
    # a = wikiSearchV2(q)
    # anames = (([x['name'] for x in a]))
    # print(', '.join(anames))
    print('end')
