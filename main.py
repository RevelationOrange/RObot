...
'''
(RevelationOrange's comments)
base code:
credit to qndel for getting this started, pointing me to the right examples, and showing me how to deal with xmpp messages and some event handlers
...
# encrypted password goes in the second quotes
user_password = '{"k":""}'

room_jid = "174726-swords-and-potions-2-1@conference.of1.kongregate.com"
room_nick = "snp2_RObot"

chatlogFilename = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'logs' + os.sep + time.strftime('%m.%d.%Y.txt')
bunnylogFilename = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'logs' + os.sep + 'bunnycuddles.txt'
...
class MUCBot(sleekxmpp.ClientXMPP):
    ...
    def __init__(self, jid, password, room, nick, logfilename='', repliesEnabled=False, bunnyLog=''):
        ...
        self.pmCmdTimers = {}
        self.hinderances = {}
        self.publicCooldowns = {'pb': 0, 'sheets': 0, 'gs': 0, 'fwt': 0, 'wiki': 0, 'refresh': 0, 'bwiki': 0,
                                'wtfodder':0, 'shutdown': 0, 'snp2death': 0}
            ...
            if usr not in self.pmCmdTimers or time.time() - self.pmCmdTimers[usr] > 3.:
                ...
                if usr in self.hinderances and self.hinderances[usr]:
                    if inputs[0][1:] not in ['stopHinder', 'stophinder']:
                        ...
                        i = random.randint(0, len(RObotLib.hinderRNG))
                        inputs[0] = '!' + RObotLib.hinderRNG[i]
                else:
                ...
                if handled is not None:
                    if inputs[0][1:] == 'hinder':
                        if len(inputs) > 1 and RObotLib.canIntConvert(inputs[1]):
                            nRands = inputs[1]
                        else:
                            nRands = random.randint(1,5)
                    ...
                    for i in range(len(responses)):
                        time.sleep(1.2)
                        print(color('{}  PM TO {}: {}'.format(time.strftime("%X",time.localtime()), usr, replyMsgs[i]), pmColor))
                        if self.logFilename:
    ...
    def privateMessage(self, msg):
        ...
        if tmp is not None:
            msgText, sendTo = tmp
            if '@RO'.lower() in msgText.lower():
                droid.notify(time.strftime("%X", time.localtime()), '{}: {}'.format(sendTo, msgText))
                droid.vibrate(420)
            if self.logFilename:
                with codecs.open(self.logFilename, 'a', 'utf-8') as lf:
                    lf.write('pm from,{},{},{}\n'.format(sendTo,time.time(),msgText))
            print(color('{}  PM FROM {}: {}'.format(time.strftime("%X",time.localtime()), sendTo, msgText), pmColor))
            ...
            if msgBody and sendTo != 'RevelationOrange':
                ...
                inputs = msgBody.split(' ')
                if inputs[0] == '!say':
                    chatTxt = ''.join([x for x in ' '.join(inputs[1:]) if x != '\\'])
                    ...
                    for t in self.publicCooldowns:
                        print(color("{}: {}".format(t, time.ctime(self.publicCooldowns[t])), "blue"))
                elif inputs[0] in ['!disconnect', '!dc']:
                    bye = 'see ya'
                    resp = RObotLib.formulateResponse(bye, sendTo)
                    xmpp.send_raw(resp)
                    if self.logFilename:
                        with codecs.open(self.logFilename, 'a', 'utf-8') as lf:
                            lf.write('pm to,{},{},{}\n'.format(sendTo, time.time(), bye))
                    self.logout()
                else:
                    self.handlePM(msgBody, sendTo)
    ...
    def muc_message(self, msg):
        ...
        if time.time()-self.startTime > 3.:
            ...
            if msg['body'][:len(pbMatch)] == pbMatch:
                if time.time() - self.publicCooldowns['pb'] >= grpAndPBcooldown * 60.:
            ...
            # check if problem
                ...
                if msg['body'][0] == '!':
                    ...
                    if f and time.time() - self.publicCooldowns[f] >= grpAndPBcooldown * 60.:
                        resp = RObotLib.grpMsgRepl.format(RObotLib.optionDict[f]())
                        time.sleep(delayTime)
                        xmpp.send_raw(resp)
    ...
    def muc_online(self, presence):
        self.fwtAnnounce()
        self.shutdownAnnounce()
        # print(color('{} presence: {}'.format(time.strftime("%X", time.localtime()), presence), 'blue'))
        """
        ...
        if time.time() - self.publicCooldowns['fwt'] >= 7*60.:
            ...
            if tier:
                ...
                if timeLeft > ((24*60)-5)*60. and timeLeft <= 24*60*60.:
                    resp = RObotLib.grpMsgRepl.format("Happy free world trade day! (trader's guild level {})".format(tier))
                elif timeLeft > (31-5)*60. and timeLeft <= (31)*60.:
                    resp = RObotLib.grpMsgRepl.format("FWT is almost over! {} left.".format(RObotLib.timeTextConverter(timeLeft, False)))
                elif timeLeft and timeLeft <= 60*2:
                    resp = RObotLib.grpMsgRepl.format("FWT is (basically) over! The next one will be happening in {}. Use !fwt for more info.".format(RObotLib.timeTextConverter(min(nxt1,nxt15,nxtSun), False)))
                else:
                    hrs = timeLeft/3600.
                    hrsApprox = round(hrs)
                    if not hrsApprox%3 and abs(hrs - hrsApprox)*60 < 2:
                        resp = RObotLib.grpMsgRepl.format("FWT active, ~{} hours left.".format(hrsApprox))
                    else:
                        resp = 0
                if resp:
                    xmpp.send_raw(resp)
                ...
                if nearest > (31-5)*60. and nearest <= (31)*60.:
                    ...
                    resp = RObotLib.grpMsgRepl.format("A free world trade day will be starting in {}!".format(RObotLib.timeTextConverter(nearest, False)))
                    xmpp.send_raw(resp)
                    self.publicCooldowns['fwt'] = time.time()
        ...
        if time.time() - self.publicCooldowns['shutdown'] >= 7*60:
            ...
            if ttc <= 3.*24*60*60:
                if ttcInM > (72*60)-5 and ttcInM <= 72*60:
                    ...
                    resp = "Five minutes until the moon crashes into... whatever the fuck the name of snp2's kingdom is. Good riddance, King Konrad."
                else:
                    resp = 0
                if resp:
