...
class MUCBot(sleekxmpp.ClientXMPP):
    ...
    def muc_message(self, msg):
        ...
        if time.time()-self.startTime > 3.:
            ...
            elif msg['body'].lower() in gsTxt:
                if time.time() - self.gsCooldown >= grpAndPBcooldown * 60.:
                    resp = RObotLib.grpMsgRepl.format(gsMsg)
                    time.sleep(delayTime)
                    xmpp.send_raw(resp)
                    self.gsCooldown = time.time()
                else:
                    msgBody = checkForCommands(msg['body'], 'public')
                    if msgBody:
                        self.handlePM(msgBody, msg['mucnick'])
            elif msg['body'].lower() in wikiTxt:
                if time.time() - self.wikiCooldown >= grpAndPBcooldown * 60.:
                    resp = RObotLib.grpMsgRepl.format(RObotLib.wiki())
                    time.sleep(delayTime)
                    xmpp.send_raw(resp)
                    self.wikiCooldown = time.time()
                else:
                    msgBody = checkForCommands(msg['body'], 'public')
                    if msgBody:
                        self.handlePM(msgBody, msg['mucnick'])
    ...
    def fwtAnnounce(self):
        if time.time() - self.fwtCooldown >= 7*60.:
