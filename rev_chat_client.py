...
user_jid = 'RevelationOrange@of1.kongregate.com'
user_password = '{"k":"4114074_f4056e6c45173cdd5f4cd3972f8061d7ad23521a"}'
room_jid = "174726-swords-and-potions-2-1@conference.of1.kongregate.com"
room_nick = "RevelationOrange"
grpMsgRepl = "<message to='174726-swords-and-potions-2-1@conference.of1.kongregate.com' from='RevelationOrange@of1.kongregate.com/xiff' type='groupchat' id='dc96ae0e-27ec-40e5-83f2-b4a013d5ec0a' xmlns='jabber:client'><body>{}</body><x xmlns='jabber:x:event'><composing/></x></message>"
...
class MUCBot(sleekxmpp.ClientXMPP):
    ...
    def __init__(self, jid, password, room, nick, repliesEnabled=False):
        ...
        if tmp is not None:
            ...
            if msgBody and sendTo != 'RevelationOrange':
                ...
                if inputs[0] == '!say':
                    ...
                    resprepl = "<message to='174726-swords-and-potions-2-1@conference.of1.kongregate.com' from='RevelationOrange@of1.kongregate.com/xiff' type='groupchat' id='dc96ae0e-27ec-40e5-83f2-b4a013d5ec0a' xmlns='jabber:client'><body>{}</body><x xmlns='jabber:x:event'><composing/></x></message>"
    ...
    def start(self, event):
        ...
        xmpp.send_raw("<presence from='revelationorange@of1.kongregate.com/xiff' to='174726-swords-and-potions-2-1@conference.of1.kongregate.com/RevelationOrange' xmlns='jabber:client'><x xmlns='http://jabber.org/protocol/muc'><history seconds='60'/></x><status>[&quot;Fan93JUt6GtzEChX+EQfxg==&quot;,&quot;[\&quot;RevelationOrange\&quot;,\&quot;\&quot;,11,\&quot;cdn3:/assets/avatars/defaults/pinkie.png?i10c=img.resize(width:16)\&quot;,\&quot;Swords \\\\u0026 Potions 2\&quot;,\&quot;/games/EdgebeeStudios/swords-and-potions-2\&quot;,[],[]]&quot;,{&quot;special_chat_vars&quot;:&quot;{}&quot;}]</status></presence>")
        ...
        if time.time() - self.publicCooldowns['fwt'] >= 7*60.:
            ...
            if tier:
                ...
                if timeLeft > ((24*60)-5)*60. and timeLeft <= 24*60*60.:
                    resp = grpMsgRepl.format("Happy free world trade day! (trader's guild level {})".format(tier))
                elif timeLeft > (31-5)*60. and timeLeft <= (31)*60.:
                    resp = grpMsgRepl.format("FWT is almost over! {} left.".format(RObotLib.timeTextConverter(timeLeft, False)))
                elif timeLeft and timeLeft <= 60*2:
                    resp = grpMsgRepl.format("FWT is (basically) over! The next one will be happening in {}. Use !fwt for more info.".format(RObotLib.timeTextConverter(min(nxt1,nxt15,nxtSun), False)))
                else:
                    hrs = timeLeft/3600.
                    hrsApprox = round(hrs)
                    if not hrsApprox%3 and abs(hrs - hrsApprox)*60 < 2:
                        resp = grpMsgRepl.format("FWT active, ~{} hours left.".format(hrsApprox))
                    else:
                        resp = 0
                if resp:
                    xmpp.send_raw(resp)
                ...
                if nearest > (31-5)*60. and nearest <= (31)*60.:
                    ...
                    resp = grpMsgRepl.format("A free world trade day will be starting in {}!".format(RObotLib.timeTextConverter(nearest, False)))
                    xmpp.send_raw(resp)
                    self.publicCooldowns['fwt'] = time.time()
